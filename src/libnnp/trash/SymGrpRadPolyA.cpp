...
SymGrpRadPolyA::
SymGrpRadPolyA(ElementMap const& elementMap) : SymGrp(280, elementMap),
    e1(0),
    rl(0)
{
...
bool SymGrpRadPolyA::operator==(SymGrp const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymGrpRadPolyA const& c = dynamic_cast<SymGrpRadPolyA const&>(rhs);
...
bool SymGrpRadPolyA::addMember(SymFnc const* const symmetryFunction)
{
    if (symmetryFunction->getType() != type) return false;

    SymFncRadPolyA const* sf =
        dynamic_cast<SymFncRadPolyA const*>(symmetryFunction);
...
void SymGrpRadPolyA::sortMembers()
{
    sort(members.begin(),
         members.end(),
         comparePointerTargets<SymFncRadPolyA const>);
...
void SymGrpRadPolyA::setScalingFactors()
{
...
void SymGrpRadPolyA::calculate(Atom& atom, bool const derivatives) const
{
...
vector<string> SymGrpRadPolyA::parameterLines() const
{
    vector<string> v;
