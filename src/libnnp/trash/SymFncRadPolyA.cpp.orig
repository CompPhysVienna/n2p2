...
SymmetryFunctionRadialPolyA::SymmetryFunctionRadialPolyA(ElementMap const& elementMap) :
    SymmetryFunction(280, elementMap),
    e1              (0  ),
    rl              (0.0)
{
    minNeighbors = 1;
...
bool SymmetryFunctionRadialPolyA::operator==(SymmetryFunction const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymmetryFunctionRadialPolyA const& c =
        dynamic_cast<SymmetryFunctionRadialPolyA const&>(rhs);
...
void SymmetryFunctionRadialPolyA::setParameters(string const& parameterString)
{
    ...
    c.setType(CompactFunction::Type::POLYA);
...
void SymmetryFunctionRadialPolyA::changeLengthUnit(double convLength)
{
...
string SymmetryFunctionRadialPolyA::getSettingsLine() const
{
...
bool SymmetryFunctionRadialPolyA::getCompactOnly(double x, double& fx, double& dfx) const
{
    bool const stat = c.fdf(x, fx, dfx);
    return stat;
}

void SymmetryFunctionRadialPolyA::calculate(Atom&  atom,
...
string SymmetryFunctionRadialPolyA::parameterLine() const
{
    int const    izero = 0;
...
vector<string> SymmetryFunctionRadialPolyA::parameterInfo() const
{
...
double SymmetryFunctionRadialPolyA::calculateRadialPart(double distance) const
{
...
double SymmetryFunctionRadialPolyA::calculateAngularPart(double /* angle */) const
{
    return 1.0;
}

bool SymmetryFunctionRadialPolyA::checkRelevantElement(size_t index) const
{
