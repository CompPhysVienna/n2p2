...
bool SymGrpAngnExp::operator==(SymGrp const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymGrpAngnExp const& c = dynamic_cast<SymGrpAngnExp const&>(rhs);
...
bool SymGrpAngnExp::addMember(SymFnc const* const symmetryFunction)
{
    if (symmetryFunction->getType() != type) return false;

    SymFncAngnExp const* sf =
        dynamic_cast<SymFncAngnExp const*>(symmetryFunction);
...
void SymGrpAngnExp::sortMembers()
{
    sort(members.begin(),
         members.end(),
         comparePointerTargets<SymFncAngnExp const>);
...
void SymGrpAngnExp::setScalingFactors()
{
...
void SymGrpAngnExp::calculate(Atom& atom, bool const derivatives) const
{
...
vector<string> SymGrpAngnExp::parameterLines() const
{
    vector<string> v;
