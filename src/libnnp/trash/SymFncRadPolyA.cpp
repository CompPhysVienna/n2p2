...
SymFncRadPolyA::SymFncRadPolyA(ElementMap const& elementMap) :
    SymFnc(280, elementMap),
    e1              (0  ),
    rl              (0.0)
{
    minNeighbors = 1;
...
bool SymFncRadPolyA::operator==(SymFnc const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymFncRadPolyA const& c = dynamic_cast<SymFncRadPolyA const&>(rhs);
...
void SymFncRadPolyA::setParameters(string const& parameterString)
{
...
void SymFncRadPolyA::changeLengthUnit(double convLength)
{
...
string SymFncRadPolyA::getSettingsLine() const
{
...
bool SymFncRadPolyA::getCompactOnly(double x, double& fx, double& dfx) const
{
    bool const stat = c.fdf(x, fx, dfx);
    return stat;
}

void SymFncRadPolyA::calculate(Atom& atom, bool const derivatives) const
{
    double result = 0.0;
...
string SymFncRadPolyA::parameterLine() const
{
    int const    izero = 0;
...
vector<string> SymFncRadPolyA::parameterInfo() const
{
...
double SymFncRadPolyA::calculateRadialPart(double distance) const
{
...
double SymFncRadPolyA::calculateAngularPart(double /* angle */) const
{
    return 1.0;
}

bool SymFncRadPolyA::checkRelevantElement(size_t index) const
{
