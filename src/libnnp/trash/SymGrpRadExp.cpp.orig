...
SymGrpRadExp::
SymGrpRadExp(ElementMap const& elementMap) : SymGrp(2, elementMap),
    e1(0)
{
...
bool SymGrpRadExp::operator==(SymGrp const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymGrpRadExp const& c = dynamic_cast<SymGrpRadExp const&>(rhs);
...
bool SymGrpRadExp::addMember(SymFnc const* const symmetryFunction)
{
    if (symmetryFunction->getType() != type) return false;

    SymFncRadExp const* sf =
        dynamic_cast<SymFncRadExp const*>(symmetryFunction);
...
void SymGrpRadExp::sortMembers()
{
    sort(members.begin(),
         members.end(),
         comparePointerTargets<SymFncRadExp const>);
...
void SymGrpRadExp::setScalingFactors()
{
...
void SymGrpRadExp::calculate(Atom& atom, bool const derivatives) const
{
...
vector<string> SymGrpRadExp::parameterLines() const
{
    vector<string> v;
