...
bool SymGrpRadExpWeighted::operator==(SymGrp const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymGrpRadExpWeighted const& c =
        dynamic_cast<SymGrpRadExpWeighted const&>(rhs);
...
bool SymGrpRadExpWeighted::addMember(SymFnc const* const symmetryFunction)
{
    if (symmetryFunction->getType() != type) return false;

    SymFncRadExpWeighted const* sf =
        dynamic_cast<SymFncRadExpWeighted const*>(symmetryFunction);
...
void SymGrpRadExpWeighted::sortMembers()
{
    sort(members.begin(),
         members.end(),
         comparePointerTargets<SymFncRadExpWeighted const>);
...
void SymGrpRadExpWeighted::setScalingFactors()
{
...
void SymGrpRadExpWeighted::calculate(Atom& atom, bool const derivatives) const
{
...
vector<string> SymGrpRadExpWeighted::parameterLines() const
{
    vector<string> v;
