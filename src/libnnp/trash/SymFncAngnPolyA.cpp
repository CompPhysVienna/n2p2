...
SymFncAngnPolyA::
SymFncAngnPolyA(ElementMap const& elementMap) : SymFnc(990, elementMap),
    e1           (0    ),
    e2           (0    ),
    rl           (0.0  ),
    angleLeft    (0.0  ),
...
bool SymFncAngnPolyA::operator==(SymFnc const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymFncAngnPolyA const& c = dynamic_cast<SymFncAngnPolyA const&>(rhs);
...
void SymFncAngnPolyA::setParameters(string const& parameterString)
{
...
void SymFncAngnPolyA::changeLengthUnit(double convLength)
{
...
string SymFncAngnPolyA::getSettingsLine() const
{
...
bool SymFncAngnPolyA::getCompactAngle(double x, double& fx, double& dfx) const
{
    bool const stat = ca.fdf(x, fx, dfx);
    return stat;
}

bool SymFncAngnPolyA::getCompactRadial(double x, double& fx, double& dfx) const
{
    bool const stat = cr.fdf(x, fx, dfx);
    return stat;
}

void SymFncAngnPolyA::calculate(Atom& atom, bool const derivatives) const
{
...
string SymFncAngnPolyA::parameterLine() const
{
    int const    izero = 0;
...
vector<string> SymFncAngnPolyA::parameterInfo() const
{
...
double SymFncAngnPolyA::calculateRadialPart(double distance) const
{
...
double SymFncAngnPolyA::calculateAngularPart(double angle) const
{
    return ca.f(angle);
}

bool SymFncAngnPolyA::checkRelevantElement(size_t index) const
{
