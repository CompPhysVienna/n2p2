...
SymGrpAngnPolyA::
SymGrpAngnPolyA(ElementMap const& elementMap) : SymGrp(990, elementMap),
    e1(0),
    e2(0)
{
...
bool SymGrpAngnPolyA::operator==(SymGrp const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymGrpAngnPolyA const& c = dynamic_cast<SymGrpAngnPolyA const&>(rhs);
...
bool SymGrpAngnPolyA::addMember(SymFnc const* const symmetryFunction)
{
    if (symmetryFunction->getType() != type) return false;

    SymFncAngnPolyA const* sf =
        dynamic_cast<SymFncAngnPolyA const*>(symmetryFunction);
...
void SymGrpAngnPolyA::sortMembers()
{
    sort(members.begin(),
         members.end(),
         comparePointerTargets<SymFncAngnPolyA const>);
...
void SymGrpAngnPolyA::setScalingFactors()
{
...
void SymGrpAngnPolyA::calculate(Atom& atom, bool const derivatives) const
{
...
vector<string> SymGrpAngnPolyA::parameterLines() const
{
    vector<string> v;
    int const    izero = 0.0;
