...
SymFncAngwPolyA::
SymFncAngwPolyA(ElementMap const& elementMap) : SymFnc(890, elementMap),
    e1           (0    ),
    e2           (0    ),
    rl           (0.0  ),
    angleLeft    (0.0  ),
...
bool SymFncAngwPolyA::operator==(SymFnc const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymFncAngwPolyA const& c = dynamic_cast<SymFncAngwPolyA const&>(rhs);
...
void SymFncAngwPolyA::setParameters(string const& parameterString)
{
...
void SymFncAngwPolyA::changeLengthUnit(double convLength)
{
...
string SymFncAngwPolyA::getSettingsLine() const
{
...
bool SymFncAngwPolyA::getCompactAngle(double x, double& fx, double& dfx) const
{
    bool const stat = ca.fdf(x, fx, dfx);
    return stat;
}

bool SymFncAngwPolyA::getCompactRadial(double x, double& fx, double& dfx) const
{
    bool const stat = cr.fdf(x, fx, dfx);
    return stat;
}

void SymFncAngwPolyA::calculate(Atom& atom, bool const derivatives) const
{
...
string SymFncAngwPolyA::parameterLine() const
{
    int const    izero = 0;
...
vector<string> SymFncAngwPolyA::parameterInfo() const
{
...
double SymFncAngwPolyA::calculateRadialPart(double distance) const
{
...
double SymFncAngwPolyA::calculateAngularPart(double angle) const
{
    return ca.f(angle);
}

bool SymFncAngwPolyA::checkRelevantElement(size_t index) const
{
