...
SymGrpAngnExpWeighted::
SymGrpAngnExpWeighted(ElementMap const& elementMap) : SymGrp(13, elementMap)
{
...
bool SymGrpAngnExpWeighted::operator==(SymGrp const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymGrpAngnExpWeighted const& c =
        dynamic_cast<SymGrpAngnExpWeighted const&>(rhs);
...
bool SymGrpAngnExpWeighted::addMember(SymFnc const* const symmetryFunction)
{
    if (symmetryFunction->getType() != type) return false;

    SymFncAngnExpWeighted const* sf =
        dynamic_cast<SymFncAngnExpWeighted const*>(symmetryFunction);
...
void SymGrpAngnExpWeighted::sortMembers()
{
    sort(members.begin(),
         members.end(),
         comparePointerTargets<SymFncAngnExpWeighted const>);
...
void SymGrpAngnExpWeighted::setScalingFactors()
{
...
void SymGrpAngnExpWeighted::calculate(Atom& atom, bool const derivatives) const
{
...
vector<string> SymGrpAngnExpWeighted::parameterLines() const
{
    vector<string> v;
