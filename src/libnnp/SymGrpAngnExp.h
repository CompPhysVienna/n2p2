 ...
 * - @f$\alpha@f$
 */
class SymGrpAngnExp : public SymGrp
{
public:
    /** Constructor, sets type = 3
     */
    SymGrpAngnExp(ElementMap const& elementMap);
    /** Overload == operator.
     */
...
private:
    ...
    std::vector<SymFncAngnExp const*> members;
...
inline bool SymGrpAngnExp::operator!=(SymGrp const& rhs) const
{
    return !((*this) == rhs);
}
