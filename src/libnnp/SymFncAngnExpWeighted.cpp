...
SymFncAngnExpWeighted::SymFncAngnExpWeighted(ElementMap const& elementMap) :
    SymFncCutoffBased(13, elementMap),
    useIntegerPow(false),
    zetaInt      (0    ),
    eta          (0.0  ),
    rs           (0.0  ),
...
bool SymFncAngnExpWeighted::operator==(SymFnc const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymFncAngnExpWeighted const& c =
        dynamic_cast<SymFncAngnExpWeighted const&>(rhs);
...
void SymFncAngnExpWeighted::setParameters(string const& parameterString)
{
...
void SymFncAngnExpWeighted::changeLengthUnit(double convLength)
{
...
string SymFncAngnExpWeighted::getSettingsLine() const
{
...
void SymFncAngnExpWeighted::calculate(Atom& atom, bool const derivatives) const
{
...
string SymFncAngnExpWeighted::parameterLine() const
{
...
vector<string> SymFncAngnExpWeighted::parameterInfo() const
{
...
double SymFncAngnExpWeighted::calculateRadialPart(double distance) const
{
...
double SymFncAngnExpWeighted::calculateAngularPart(double angle) const
{
    return 2.0 * pow((1.0 + lambda * cos(angle)) / 2.0, zeta);
}

bool SymFncAngnExpWeighted::checkRelevantElement(size_t /*index*/) const
{
    return true;
}
