...
SymmetryFunctionAngularPolyOnly::
SymmetryFunctionAngularPolyOnly(ElementMap const& elementMap) :
    SymmetryFunction(89, elementMap),
    e1           (0    ),
    e2           (0    ),
    rl           (0.0  ),
    angleLeft    (0.0  ),
...
bool SymmetryFunctionAngularPolyOnly::
operator==(SymmetryFunction const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymmetryFunctionAngularPolyOnly const& c =
        dynamic_cast<SymmetryFunctionAngularPolyOnly const&>(rhs);
...
void SymmetryFunctionAngularPolyOnly::
...
void SymmetryFunctionAngularPolyOnly::changeLengthUnit(double convLength)
{
...
string SymmetryFunctionAngularPolyOnly::getSettingsLine() const
{
...
bool SymmetryFunctionAngularPolyOnly::getCompactAngle(double x, double& fx, double& dfx) const
{
    bool const stat = ca.fdf(x, fx, dfx);
    return stat;
}

bool SymmetryFunctionAngularPolyOnly::getCompactRadial(double x, double& fx, double& dfx) const
{
    bool const stat = cr.fdf(x, fx, dfx);
    return stat;
}

void SymmetryFunctionAngularPolyOnly::calculate(Atom&      atom,
...
vector<string> SymmetryFunctionAngularPolyOnly::parameterInfo() const
{
...
double SymmetryFunctionAngularPolyOnly::calculateRadialPart(double distance) const
{
...
double SymmetryFunctionAngularPolyOnly::calculateAngularPart(double angle) const
{
    return ca.f(angle);
}

bool SymmetryFunctionAngularPolyOnly::checkRelevantElement(size_t index) const
{
