...
SymGrpAngwPolyA::
SymGrpAngwPolyA(ElementMap const& elementMap) : SymGrp(890, elementMap),
    e1(0),
    e2(0)
{
...
bool SymGrpAngwPolyA::operator==(SymGrp const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymGrpAngwPolyA const& c = dynamic_cast<SymGrpAngwPolyA const&>(rhs);
...
bool SymGrpAngwPolyA::addMember(SymFnc const* const symmetryFunction)
{
    if (symmetryFunction->getType() != type) return false;

    SymFncAngwPolyA const* sf = dynamic_cast<SymFncAngwPolyA const*>(
        symmetryFunction);
...
void SymGrpAngwPolyA::sortMembers()
{
    sort(members.begin(),
         members.end(),
         comparePointerTargets<SymFncAngwPolyA const>);
...
void SymGrpAngwPolyA::setScalingFactors()
{
...
void SymGrpAngwPolyA::calculate(Atom& atom, bool const derivatives) const
{
...
vector<string> SymGrpAngwPolyA::parameterLines() const
{
    vector<string> v;
    int const    izero = 0.0;
