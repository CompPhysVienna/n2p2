...
SymGrpAngwExp::SymGrpAngwExp(ElementMap const& elementMap) :
    SymGrpCutoffBased(9, elementMap),
    e1(0),
    e2(0)
{
...
bool SymGrpAngwExp::operator==(SymGrp const& rhs) const
{
    if (ec   != rhs.getEc()  ) return false;
    if (type != rhs.getType()) return false;
    SymGrpAngwExp const& c = dynamic_cast<SymGrpAngwExp const&>(rhs);
...
bool SymGrpAngwExp::addMember(SymFnc const* const symmetryFunction)
{
    if (symmetryFunction->getType() != type) return false;

    SymFncAngwExp const* sf =
        dynamic_cast<SymFncAngwExp const*>(symmetryFunction);
...
void SymGrpAngwExp::sortMembers()
{
    sort(members.begin(),
         members.end(),
         comparePointerTargets<SymFncAngwExp const>);
...
void SymGrpAngwExp::setScalingFactors()
{
...
void SymGrpAngwExp::calculate(Atom& atom, bool const derivatives) const
{
...
vector<string> SymGrpAngwExp::parameterLines() const
{
    vector<string> v;
